        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in-up animation-delay-400">
          {filteredItems.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className="group relative bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-cyan-400/30 hover:shadow-2xl hover:shadow-cyan-500/20 transition-all duration-700 hover:-translate-y-4 hover:scale-105 hover:rotate-1 cursor-pointer"
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden">
                  ...
                  {/* Category Badge */}
                  <div className="absolute top-4 left-4 px-3 py-1 bg-cyan-400/30 backdrop-blur-sm text-cyan-300 rounded-full text-xs font-medium border border-cyan-400/50 group-hover:scale-110 group-hover:bg-cyan-400/50 transition-all duration-300">
                    {categories.find(cat => cat.id === item.category)?.label}
                  </div>

                  {/* Hover Overlay - Now uses the correct icon for each category */}
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500">
                    <div className="bg-white/30 backdrop-blur-sm rounded-full p-6 group-hover:scale-110 group-hover:rotate-12 transition-all duration-300">
                ...
                {/* Content */}
                <div className="p-6">
                  <h3 className="text-lg font-bold text-white mb-2 group-hover:text-cyan-400 group-hover:scale-105 transition-all duration-300 line-clamp-2">
                    {item.title}
                  </h3>
                  <div className="flex items-center gap-4 text-sm text-gray-400 mb-3">
                    <div className="flex items-center gap-1">
                      <Calendar size={14} />
                      <span className="group-hover:text-cyan-400 transition-colors duration-300">{item.date}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <MapPin size={14} />
                      <span className="truncate group-hover:text-purple-400 transition-colors duration-300">{item.location}</span>
                    </div>
                  </div>
